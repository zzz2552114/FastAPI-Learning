...
## 目录
- [模块一：基础框架与 跨域 API 设计 (Routing & CORS)](#模块一基础框架与-跨域-api-设计-routing--cors)
...
## 模块一：基础框架与 跨域 API 设计 (Routing & CORS)

### 1. 深度补充学习与底层原理解析

#### 1.1 ASGI 协议与 WSGI 协议的性能断代
在传统的 Python Web 开发中，**WSGI (Web Server Gateway Interface)**（如 Flask、Django）采用的是同步阻塞式架构。每个传入 of the HTTP 连接都会被分配给一个独立的线程。当遇到 I/O 操作（例如从磁盘读取 Markdown 文件、查询数据库等）时，该线程会被操作系统挂起，直到 I/O 数据就绪。这种模式在应对海量并发连接（如长连接、高频轮询）时，会由于线程上下文切换（Context Switch）的开销以及线程数量限制而迅速达到物理瓶颈。
...
#### 1.4 前后端分离核心：CORS 跨域资源共享中间件原理与配置
在前后端分离架构中，前端网页（如运行在 `http://localhost:5500`）和后端 API 服务（如运行在 `http://localhost:8000`）通常处于不同的**源（Origin）**下。根据浏览器的**同源策略（Same-Origin Policy）**，浏览器会默认阻止跨源的 AJAX/Fetch 请求，除非后端服务器显式声明允许该跨域行为。

FastAPI 内置了 `CORSMiddleware` 用来处理跨域资源共享（CORS）问题：
* **工作机制**：当浏览器发起非简单跨域请求（如带有自定义 Header 或 `application/json` 类型的 `POST`/`PUT`）时，浏览器会首先自动发送一个预检请求（Preflight Request，方法为 `OPTIONS`）来询问服务器是否允许该跨域操作。
* **中间件拦截**：FastAPI 的 `CORSMiddleware` 会拦截这些 `OPTIONS` 请求，并根据配置的规则在响应头中附加 `Access-Control-Allow-Origin`、`Access-Control-Allow-Methods`、`Access-Control-Allow-Headers` 等字段。若配置通过，浏览器才会继续发送实际的业务请求。
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:5500", "http://127.0.0.1:5500"], # 允许访问的独立前端源列表
...
#### 【主线一】个人博客系统 - Phase 1：本地 Markdown 跨域解析器

##### 2.1.1 目录结构与准备工作
由于采用前后端分离架构，项目代码在物理上完全解耦，可放置在不同目录下独立开发和运行。
```text
# 后端 API 服务目录
blog_phase1_backend/
├── main.py
...
# 前端静态页面目录（可由 Live Server 等独立托管）
blog_phase1_frontend/
└── index.html
```
请在本地创建如上目录，并在 `posts/1.md` 和 `posts/2.md` 中写入简短的 Markdown 原文。

##### 2.1.2 路由架构设计
本阶段后端仅提供纯粹的 JSON 数据接口，**不负责渲染或返回任何 HTML 页面**：
1. `GET /api/v1/posts`：以 JSON 格式返回本地 `posts/` 目录下所有可用的 Markdown 文章 ID 列表。
2. `GET /api/v1/posts/{post_id}`：根据文章 ID 读取并返回该 Markdown 文件的详细纯文本内容。

##### 2.1.3 开发引导流水线
* **第一步：主路径计算与配置**  
  在 `main.py` 中初始化 `FastAPI` 实例。使用 `os.path.dirname(os.path.abspath(__file__))` 锁定当前文件的绝对路径，并派生出 `posts/` 目录的绝对路径，避免在多级目录下运行时出现寻址错误。
* **第二步：配置 CORS 跨域通信中间件**  
  导入 `CORSMiddleware`。将前端独立启动的域名和端口（例如本地静态服务器 `http://127.0.0.1:5500`）注册到 `allow_origins` 中，确保前端可以通过 `fetch` 正常请求本 API 服务。
* **第三步：实现文章列表 API**  
  定义接口 `list_posts`。此步骤需要调用本地同步磁盘读写，因此请考虑：它应该被声明为 `def` 还是 `async def`？使用 `os.listdir` 遍历 `posts/` 目录，将后缀为 `.md` 的文件剥离出 ID，并对其进行排序后以 JSON 格式 `{"posts": [...]}` 返回给前端。
* **第四步：实现单篇文章获取 API**  
  定义接口 `get_post`，接收一个路径参数 `post_id: str`。在函数体内拼接出实际的文件绝对路径，读取纯文本，以 `{"id": post_id, "content": "..."}` 格式返回。
...
* **静态部署说明**：
  * 前端网页 `index.html` 独立放置于 `blog_phase1_frontend` 目录中，可以使用 VS Code 的 `Live Server` 插件（默认运行在 `http://127.0.0.1:5500`）独立启动。
* **布局设计（极简主义）**：
  * **标题区**：一个大标题 `我的 Markdown 博客 (Client)`。
  * **列表区**：一个按钮容器（`div`），用于动态挂载从后端获取的文章列表按钮。
...
* **外部依赖引入**：
  * 在 `<head>` 中通过 CDN 引入 `marked.min.js`，用来将 Markdown 文本实时转化为 HTML：
    ...
    ```
* **原生 JavaScript 跨域通信控制流程**：
  1. **跨域初始化加载**：页面加载时（`window.onload`），通过原生 `fetch('http://127.0.0.1:8000/api/v1/posts')` 跨域获取所有文章 ID 列表。
  2. **动态生成 DOM**：遍历获取到的 ID 列表，使用 `document.createElement('button')` 动态创建按钮，并将它们挂载到列表容器中。
  3. **事件绑定与获取正文**：为每个按钮绑定点击事件。当点击按钮时，发起 `fetch('http://127.0.0.1:8000/api/v1/posts/' + id)` 异步跨域请求。
  4. **Markdown 解析与挂载**：获取到后端返回的 `content` 字符串后，调用 `marked.parse(content)` 转换为 HTML，并直接赋值给渲染区的 `innerHTML`。

---

#### 【主线二】事件竞猜与预测市场 - Phase 1：内存竞猜跨域骨架

##### 2.2.1 目录结构与准备工作
```text
# 后端 API 服务目录
prediction_phase1_backend/
└── main.py

# 前端静态页面目录
prediction_phase1_frontend/
└── index.html
```

##### 2.2.2 路由架构设计
本阶段后端仅提供内存级 API，不渲染页面：
1. `GET /api/v1/predictions/{event_id}`：获取特定竞猜事件当前的奖池与状态。
2. `POST /api/v1/predictions/{event_id}/bet`：提交下注，接收查询参数 `user_id: str`、`option: str`（如 "yes" 或 "no"）与 `amount: float`。
...
* **第一步：构建内存虚拟数据库**  
  ...
  ```
* **第二步：配置 CORS 跨域资源共享**  
  在 FastAPI 实例中注册 `CORSMiddleware`。将前端静态服务器源（如 `http://127.0.0.1:5500`）加入 `allow_origins` 中，确保跨域下注请求不会被浏览器拦截。
* **第三步：实现查询 API**  
  定义接口 `get_event`。由于该操作仅读取内存字典，属于微秒级 CPU 操作，**请将其声明为 `async def`**，使其直接在主事件循环中完成，无任何额外线程开销。
* **第四步：实现下注 API**  
  定义接口 `place_bet`。接收路径参数 `event_id`，以及查询参数 `user_id`、`option` 和 `amount`。该接口会对指定的竞猜事件选项累加下注金额，并递增下注计数，以 JSON 格式返回最新事件状态。
...
* **静态部署说明**：
  * 前端网页独立运行于前端目录，通过 HTTP 跨域向运行在 `http://127.0.0.1:8000` 的后端请求数据。
* **布局设计（极简主义）**：
  * **事件看板**：用加粗标题显示从后端动态加载的“竞猜问题”（如：明天会下雨吗？）。
  * **状态展示板**：实时展示“Yes 选项总额”和“No 选项总额”，以及“总投注次数”。
  * **交互区**：包含一个“用户ID”文本输入框、一个“下注金额”数字输入框，以及两个分别标注为“押 Yes”和“押 No”的按钮。
* **原生 JavaScript 跨域通信控制流程**：
  1. **定时跨域轮询**：定义 `updateEventStatus` 函数，通过 `fetch('http://127.0.0.1:8000/api/v1/predictions/tomorrow-rain')` 周期性（如每隔 2 秒）跨域拉取并更新屏幕上的各个选项奖池金额和总下注次数。
  2. **提报下注请求**：点击“押 Yes”或“押 No”按钮时，提取输入框中的用户ID和下注金额，拼装对应的 URL 查询参数，跨域向 `http://127.0.0.1:8000/api/v1/predictions/tomorrow-rain/bet` 发起 `POST` 请求。
...
#### 【主线一】个人博客系统 - Phase 2：跨域安全上传与结构化解析
...
blog_phase2_frontend/
└── index.html
```

##### 2.1.2 路由架构设计
新增高危 API：`POST /api/v1/posts/upload`  
入参要求（通过 `multipart/form-data` 接收）：
...
* **第四步：利用 DTO 进行结构化对齐**  
  解析 Form 表单数据，将 `tags` 字符串切片并剔除空白字符，最终传入 `PostMetadata` 进行实例化校验，并返回标准的 JSON 结构表示上传成功。

##### 2.1.4 关键逻辑与代码健壮性防线
* **防线一：Directory Traversal（目录穿越漏洞）防御**  
  攻击者可能通过构造恶意请求将文件名伪装为 `../../etc/passwd` 或 `../main.py`，企图覆盖服务器的关键代码。
...
* **原生 JavaScript 跨域上传通信控制流程**：
  ...
  3. **发送跨域 multipart 请求**：发起 `fetch` 请求，目标地址为 `http://127.0.0.1:8000/api/v1/posts/upload`，方法为 `POST`，直接将 `formData` 作为请求体（`body`）发送。**注意**：使用 `FormData` 时，浏览器会自动配置正确的 `Content-Type: multipart/form-data` 及 boundary 分隔符，在原生 `fetch` 中不要手动指定 `Content-Type` 请求头。
...
前后端物理分离结构：
```text
prediction_phase2_backend/
└── main.py

prediction_phase2_frontend/
└── index.html
```

##### 2.2.2 路由架构设计
1. `POST /api/v1/admin/predictions`：管理员创建新的竞猜事件，接收 JSON 请求体。
2. `POST /api/v1/predictions/{event_id}/bet`：用户下注，接收 JSON 请求体。
...
* **原生 JavaScript 跨域通信控制流程**：
  1. **提交创建事件**：提取创建表单中的值，组装为 JSON 并指定 `'Content-Type': 'application/json'` 后，跨域发送 `POST` 请求至 `http://127.0.0.1:8000/api/v1/admin/predictions`。
  2. **提报非结构化投注**：提取输入框中的原始字符串值。组装成 JSON 数据：`JSON.stringify({ user_id: "user_01", option: "yes", raw_amount: rawInput })` 跨域发送至 `http://127.0.0.1:8000/api/v1/predictions/tomorrow-rain/bet`。
...
blog_phase3_frontend/
└── index.html
```

##### 2.1.2 路由架构设计
1. 新增安全受控阅读 API：`GET /api/v1/posts/{post_id}/read-secured`
2. 读者标识注入：在进入路由前，必须在请求头中携带一个识别身份的 Header：`X-Reader-Id`。
3. 动态响应更新：路由处理完后，需要将当前读者的“升级后等级”与“总 XP 经验”注入到 Response 响应头（`X-Reader-Level`, `X-Reader-XP`）中返回给客户端。
   * **CORS 配置要点**：由于前后端分离，默认情况下浏览器不允许前端 JavaScript 读取非标准响应头。后端跨域中间件配置中必须显式设置 `expose_headers=["X-Reader-Level", "X-Reader-XP"]` 暴露这些自定义头部。
...
* **第三步：编写 Callable 调用逻辑**  
  ...
  * **利用 `yield` 将控制权交还给路由**。
  * **在 `yield` 执行完毕后的后续流程里**，利用数学公式换算出用户的等级并写入 `response.headers["X-Reader-Level"]`。
* **第四步：在主路由上装载实例**  
  在 `main.py` 中业务路由上使用 `active_reader: str = Depends(reader_xp_tracker)` 挂载此依赖项。
...
* **原生 JavaScript 跨域通信控制流程**：
  ...
  2. 跨域拉取并提取响应头：向 `http://127.0.0.1:8000/api/v1/posts/{id}/read-secured` 发送请求。fetch 请求成功后，通过 `res.headers.get('X-Reader-Level')` 取出后端在 `yield` 后置链路中返回的等级指标，并将其渲染到前端徽章卡板中。
...
##### 2.2.1 目录结构与依赖
```text
prediction_phase3_backend/
├── app/
│   ├── __init__.py
│   └── dependencies.py
└── main.py

prediction_phase3_frontend/
└── index.html
```
...
* **布局设计**：
  * 在页面中央显式提供一个下注提示框。
* **原生 JavaScript 跨域通信控制流程**：
  * 用户连续点击“押注”按钮提交请求。若 fetch 请求返回的状态码是 `429`，则在前端页面弹窗展示 `detail` 返回的“下注太频繁”警告，且让下注按钮置灰禁用 3 秒。
...
* **非对称加密签名 (RS256)**：授权服务器持有高防密级的**私钥（Private Key）**专门负责签发，而外部业务网关 or 第三方微服务只分发持有**公钥（Public Key）**用来验签。公钥即使被攻击者窃取，由于单向陷门函数的不可逆性，他也绝对无法逆向仿冒签发令牌。这极大地收敛了高危写操作的攻击面。
...
#### 【主线一】个人博客系统 - Phase 4：博主管理后台跨域 JWT 拦截壁垒
...
blog_phase4_frontend/
├── login.html
└── write.html
```

##### 2.1.2 路由架构设计
后端提供标准 API 接口，完全隔离页面展示：
1. `POST /api/v1/auth/login`：博主账户核验，成功则下发 JWT Access Token。
2. `POST /api/v1/posts/secured-publish`：管理员高危发布接口，必须通过 JWT 验签拦截。
   * **CORS 配置要点**：由于下发与校验涉及客户端在 Header 写入带有 `'Authorization': 'Bearer ...'` 的请求头，后端的 `CORSMiddleware` 的 `allow_headers` 必须允许该请求头的跨域传递。

##### 2.1.3 开发引导流水线
* **第一步：建立认证底座**  
  在 `main.py` 顶部引入 `fastapi.security` 模块中的 `OAuth2PasswordBearer`，设定解析 `Authorization: Bearer <Token>` 头部。
* **第二步：编写 JWT 生产机器**  
  使用 `jose.jwt` 构建 `create_access_token` 生成器。包含 `sub`（用户名）与 `exp`（过期截止时间）。
* **第三步：构建管理员鉴权依赖函数**  
  编写 `get_current_admin` 函数作为路由依赖：
  * 接收自动解析出的 Bearer Token。
  * 调用 `jwt.decode` 解密并判断载荷中 `role` 是否为 `"admin"`。
* **第四步：给发布路由上锁**  
  在 `secured-publish` 路由中声明入参 `admin_user = Depends(get_current_admin)`。不满足权限要求的请求将被直接阻断。
...
##### 2.1.5 前端原生 JS 交互与布局蓝图
* **布局设计（前后端完全解耦）**：
  * **独立登录页（login.html）**：输入账号 `admin` 与密码，并附有“登录”按钮。
  * **独立撰写页（write.html）**：提供标题、正文文本输入域，并有一个“发布博文”按钮。
* **原生 JavaScript 跨域认证控制流程**：
  1. **登录并存储凭证**：在独立登录页面。fetch 发起跨域 POST 提交至 `http://127.0.0.1:8000/api/v1/auth/login`。
  2. **本地落盘**：核验通过后，在 JS 中执行 `localStorage.setItem("authToken", data.access_token)` 暂存本地。
  3. **受控资源携带访问**：在独立撰写页面，点击“发布博文”时，先取出 Token，在 fetch 的 headers 中，手动拼装并携带 `'Authorization': 'Bearer ' + token` 头部跨域提交至 `http://127.0.0.1:8000/api/v1/posts/secured-publish`。
...
##### 2.2.1 目录结构与核心依赖
```text
prediction_phase4_backend/
└── main.py

prediction_phase4_frontend/
└── index.html
```
...
* **原生 JavaScript 跨域通信控制流程**：
  ...
  * 在向 `http://127.0.0.1:8000/api/v1/admin/predictions/{event_id}/settle` 接口提报时，将生成的哈希串写入 `headers: { 'X-Signature': signature, 'X-Timestamp': timestamp }` 并跨域提报。
...
#### 【主线一】个人博客系统 - Phase 5：异步数据库 API 接入与 Vue 3 跨域分离
...
blog_phase5_frontend/
└── index.html
```

##### 2.1.2 路由架构设计
后端只提供纯净的异步数据库 API，由独立运行的前端利用 Vue 3 执行拉取与状态渲染：
1. `POST /api/v1/posts/db-add`：异步将博文实体持久化保存入 SQLite 数据库。
2. `GET /api/v1/posts/db-all`：异步从数据库读出所有发布的博文信息。
...
* **第四步：实现业务落盘 API**  
  在 `main.py` 的路由中注入 `db: AsyncSession = Depends(get_db_session)`。使用统一执行范式（如 `db.add`、`await db.commit()`）来完成对数据的增、查等操作并返回对应的数据 JSON 报文。
...
##### 2.1.5 前端 Vue 3 交互与布局蓝图（正式平滑迁移至 Vue 3 跨域调用）
* **前端静态部署方式**：
  * 前端文件 `index.html` 独立托管（如托管在独立静态源 `http://127.0.0.1:5500` 下），通过独立的 Vue 3 状态管理驱动。
* **布局设计**：
  * 两栏布局：
    * **左侧：新增博文面板**（输入标题、Markdown 内容的文本域，以及一键跨域保存入库按钮）。
    * **右侧：数据库拉取展台**（列出数据库里的文章，标注物理 ID 以及入库时间）。
* **外部依赖引入**：
  ```html
  <script src="https://unpkg.com/vue@3/dist/vue.global.js"></script>
  ```
* **Vue 3 跨域状态与行为驱动逻辑（非完整代码，核心控制链）**：
  1. **状态驱动区**：在 `setup()` 中声明响应式变量（`ref`）: `posts` (文章列表数组)、`title`、`content`。
  2. **行为钩子区**：定义 `loadDatabasePosts` 异步加载方法，跨域执行 `fetch('http://127.0.0.1:8000/api/v1/posts/db-all')`，获取成功后直接更新 `posts.value` 数组，Vue 会自动在界面刷新 DOM。
  3. **生命周期挂载**：在 `onMounted()` 时自动调用初始化加载。
  4. **跨域提交表单**：定义 `saveToDatabase` 方法，通过 `fetch('http://127.0.0.1:8000/api/v1/posts/db-add')` 以 JSON 或 URLSearchParams 跨域提报，提报成功后触发调用 `loadDatabasePosts()` 实现即时自刷。
...
##### 2.2.5 前端 Vue 3 跨域交互与布局蓝图
* **布局设计**：
  * **用户资产中心**：展示当前测试用户 ID、可用金币余额（`balance`）、冻结中金币（`frozen_balance`）。
  * **竞猜交互板块**：包含下注选项和投注输入框、下注按钮。
* **Vue 3 核心状态与行为跨域驱动逻辑**：
  1. **响应状态绑定**：使用 `ref` 双向绑定金币。
  2. **跨域高频提交**：点击下注按钮触发 `POST` 请求跨域至 `http://127.0.0.1:8000/api/v1/predictions/db-bet`。后端若在悲观事务流中返回“余额不足”，立刻反馈并将用户账户余额强制校准对齐。
...
#### 【主线一】个人博客系统 - Phase 6：独立弹幕网关与容器集群化

##### 2.1.1 目录结构与准备工作
前后端物理与网络层面完全拆离。前端可利用 Nginx、S3 或本地独立静态托管，后端完全专注于业务容器化：
```text
# 后端目录
blog_phase6_backend/
├── main.py
...
# 独立前端目录
blog_phase6_frontend/
└── index.html
```

##### 2.1.2 路由架构设计
升级网络协议端点：`WebSocket /api/v1/posts/{post_id}/live-barrage`  
负责为每一篇文章单独创建持久的长连接“弹幕聊天室”，维持多路并发跨域广播。
* **跨域说明**：由于 WebSocket 协议不受浏览器同源策略（CORS）的硬性限制，前端可以直接握手。但若在握手前的 HTTP Upgrade 升级阶段需要验证 Host，FastAPI 会在安全限制上做控制。
...
##### 2.1.5 前端 Vue 3 交互与布局蓝图（WebSocket 跨域双向实时交互）
* **布局设计**：
  * **弹幕大荧幕**：黑色底色框体（具有固定高度，并有 `overflow-y: auto` 滚动条）。
  * **弹幕发布面板**：横向一整排。一个文本框用于填入吐槽文字，右侧附带一个“发射弹幕”绿色按钮。
* **Vue 3 WebSocket 核心管理流程**：
  1. **建立独立链接通道**：在 Vue 的 `onMounted` 钩子内，实例化跨域 WebSocket 握手：
     ```javascript
     const ws = new WebSocket(`ws://127.0.0.1:8000/api/v1/posts/fastapi-blog/live-barrage`);
     ```
  2. **事件流监听**：为长连接实例绑定 `ws.onmessage = (event) => {}` 监听。当监听到后端推送的广播消息时，直接将字符串追加进响应式数组 `barrageList`。
  3. **视图刷新绑定**：Vue 检测到列表增长，自动将其遍历挂载到大显示屏上。使用 `nextTick` 保持大显示屏的垂直滚动条始终卡在最底部。
  4. **双向发射**：在发送按钮的点击事件中，使用 `ws.send(messageText)`，直接通过长连接通道将其发送给后端。
...
##### 2.2.1 目录结构与准备工作
请配置以下用于生产环境容器打包与前后端物理拆离的文件：
```text
# 后端 API 容器包目录
prediction_phase6_backend/
├── gunicorn_conf.py
├── Dockerfile
├── docker-compose.yml
└── main.py
...
# 前端静态服务目录（如托管于独立静态源）
prediction_phase6_frontend/
└── index.html
```

##### 2.2.2 部署架构设计
* 运行底座：使用 Docker 将 FastAPI 部署至完全物理隔离的沙盒容器中。
...
* **Vue 3 核心状态与行为驱动逻辑**：
  1. **建立实时连接**：
     ```javascript
     const ws = new WebSocket(`ws://127.0.0.1:8000/api/v1/predictions/tomorrow-rain/odds-feed`);
     ```
...
🏆 **恭喜您！如果您脚踏实地完成了这 2 个全分离式的全栈项目（从最基础的跨域 API 构筑，到 Pydantic DTO 校验、依赖注入、HMAC 数字签名、SQLAlchemy 悲观锁并发控制、WebSocket 全双工跨域广播、最终容器集群化部署），您就已经具备了应对中高级 Python 架构开发和企业复杂场景需求的深厚底气！**