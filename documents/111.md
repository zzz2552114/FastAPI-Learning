...
先看清本阶段会改动什么，再进入后面的逐函数说明。表中的项目均为本阶段必做内容。

| 项目 | 新增或修改的后端路由 | 新增或修改的后端函数 |
|---|---|---|
| 个人博客系统 | — | — |
| 事件竞猜与预测市场 | — | — |
| 两个项目共用的基础能力 | `GET /health`<br>`GET /api/v1/info` | `build_project_info` |

> **完成方式：** 先选择一个项目完成本阶段的全部路由、函数和前端联调，再停止它，切换到另一个项目完成同一阶段。
...
**建议路由函数名：** `get_project_info`

**用户从页面上做什么：** 点击“查看项目信息”。

**这个功能为什么值得在本阶段加入：** 阶段 0 只有一个健康函数太单薄；这个接口仍然只读、无参数，却能练习返回字符串、布尔值与列表。

**输入来自哪里：** 无 path/query/body。数据来自后端固定的公开配置。

**后端必须按下面顺序处理：**

1. 调用 `build_project_info`。
2. 博客返回 `project=blog-system`，预测市场返回 `project=prediction-market`。
3. 加入 API 版本、开发模式布尔值和已开放功能列表。
4. 确认响应不含数据库 URL、密钥、本机路径。

**成功时返回什么：** 200，返回 `project`、`api_version`、`development`、`features`。

**可能失败的情况：** 通常没有业务错误；前端可能连通但连错项目。

**前端怎样调用和更新页面：** 前端读取后写进一个 `<pre>`；若 project 与当前页面不一致，显示“连错后端”。

**本功能重点函数或语法：** 普通字典和列表；不需要新库。

**本阶段仍然没有解决什么：** 它不是设置转储接口。

### 后端函数卡：`build_project_info`

**函数类型：** 纯函数

**由谁调用：** `get_project_info`

**接收什么：** 项目名、版本、开发模式、功能列表。
...
**为什么要单独写这个函数：** 路由只处理 HTTP，公开信息组织集中在一处。

**重点语法或方法：** `list(features)`、字典字面量。

**边界与限制：** 不读取或返回秘密。

### 前端函数卡：`loadProjectInfo`

**所在文件：** `scripts/main.js`

**什么时候触发：** 页面加载后或点击按钮。

**需要读取什么：** 信息区、按钮、当前页面期望的项目名。
...
**它实际使用了后端返回的哪些字段：** `project`、`api_version`、`features`。

**为什么这样写而不做得更复杂：** 只多一个按钮和一个 `<pre>`。
...
| 项目 | 新增或修改的后端路由 | 新增或修改的后端函数 |
|---|---|---|
| 个人博客系统 | `GET /api/v1/posts`<br>`GET /api/v1/posts/{post_id}`<br>`GET /api/v1/posts/{post_id}/related`<br>`GET /api/v1/posts/latest` | `find_post_by_id`<br>`build_post_summary` |
| 事件竞猜与预测市场 | `GET /api/v1/events`<br>`GET /api/v1/events/{event_id}`<br>`GET /api/v1/events/open`<br>`GET /api/v1/events/{event_id}/quote` | `find_event_by_id`<br>`calculate_quote_preview` |
| 两个项目共用的基础能力 | — | — |
...
## 本阶段新增功能：在列表/详情基础上增加静态快捷入口和纯读取计算


> **实现边界：** 不在本阶段加入 POST、认证或数据库。
**建议完成顺序：** 列表详情跑通→静态路由→相关文章→试算。
...
| 项目 | 新增或修改的后端路由 | 新增或修改的后端函数 |
|---|---|---|
| 个人博客系统 | `POST /api/v1/posts`<br>`POST /api/v1/posts/preview` | `generate_next_post_id`<br>`normalize_tags` |
| 事件竞猜与预测市场 | `POST /api/v1/events`<br>`POST /api/v1/events/{event_id}/bets` | `find_option_by_id`<br>`validate_event_options` |
| 两个项目共用的基础能力 | — | — |
...
## 本阶段新增功能：把 Pydantic 阶段从“能创建”扩展到“能预览、能清理、能复用校验”


**建议完成顺序：** 后端函数先行→`/docs` 验证→最小前端入口→错误与边界测试。
...
| 项目 | 新增或修改的后端路由 | 新增或修改的后端函数 |
|---|---|---|
| 个人博客系统 | `GET /api/v1/posts`<br>`GET /api/v1/posts/{post_id}（响应契约升级）`<br>`POST /api/v1/posts（状态码和输出升级）` | — |
| 事件竞猜与预测市场 | `GET /api/v1/events`<br>`POST /api/v1/events/{event_id}/bets（幂等原型）`<br>`GET /api/v1/events/{event_id}（响应契约升级）` | — |
| 两个项目共用的基础能力 | — | — |
...
## 本阶段新增功能：让响应模型、状态码和 Header 输入真正参与现有业务
...
| 项目 | 新增或修改的后端路由 | 新增或修改的后端函数 |
|---|---|---|
| 个人博客系统 | `PATCH /api/v1/posts/{post_id}`<br>`DELETE /api/v1/posts/{post_id}`<br>`POST /api/v1/posts/import`<br>`PUT /api/v1/posts/{post_id}` | `read_markdown_with_limit`<br>`validate_markdown_upload` |
| 事件竞猜与预测市场 | `PATCH /api/v1/events/{event_id}`<br>`POST /api/v1/events/{event_id}/close`<br>`POST /api/v1/events/{event_id}/cancel` | — |
| 两个项目共用的基础能力 | — | — |
...
| 项目 | 新增或修改的后端路由 | 新增或修改的后端函数 |
|---|---|---|
| 个人博客系统 | `POST /api/v1/posts/{post_id}/reading-events`<br>`GET /api/v1/readers/current/xp` | `get_post_or_404`<br>`get_reader_context`<br>`ReadingRewardPolicy.__call__`<br>`calculate_reader_level` |
| 事件竞猜与预测市场 | `POST /api/v1/events/{event_id}/bets` | `get_event_or_404`<br>`get_clock`<br>`get_open_event`<br>`RateLimiter.__call__`<br>`place_bet_in_memory` |
| 两个项目共用的基础能力 | — | `get_pagination` |
...
| 项目 | 新增或修改的后端路由 | 新增或修改的后端函数 |
|---|---|---|
| 个人博客系统 | `POST /api/v1/posts`<br>`PATCH /api/v1/posts/{post_id}（权限升级）`<br>`POST /api/v1/posts/{post_id}/publish` | — |
| 事件竞猜与预测市场 | `POST /api/v1/events/{event_id}/bets`<br>`POST /api/v1/admin/events`<br>`POST /api/v1/admin/events/{event_id}/close`<br>`POST /api/v1/admin/events/{event_id}/settle` | — |
| 两个项目共用的基础能力 | `POST /api/v1/auth/token`<br>`GET /api/v1/users/me` | `verify_password`<br>`authenticate_user`<br>`create_access_token`<br>`get_current_user`<br>`require_roles` |
...
## 本阶段新增功能：补齐认证链、角色权限和管理员动作


**建议完成顺序：** 后端函数先行→`/docs` 验证→最小前端入口→错误与边界测试。


#### 后端函数说明卡：`verify_password`
- **函数类型**：纯安全函数
...
| 个人博客系统 | — | — |
| 事件竞猜与预测市场 | — | — |
| 两个项目共用的基础能力 | `GET /api/v1/debug/request-context` | `add_request_context`<br>`request_id_middleware`<br>`request_timing_middleware` |
...
## 本阶段新增功能：让 CORS/中间件阶段拥有三个明确后端函数


> **实现边界：** 不记录密码、token 和完整请求体。
**建议完成顺序：** CORS→request id→timing→通用前端请求。
...
| 项目 | 新增或修改的后端路由 | 新增或修改的后端函数 |
|---|---|---|
| 个人博客系统 | `GET /api/v1/posts`<br>`GET /api/v1/posts/{post_id}`<br>`POST /api/v1/posts`<br>`PATCH /api/v1/posts/{post_id}`<br>`DELETE /api/v1/posts/{post_id}`<br>`GET /api/v1/posts/{post_id}/comments`<br>`POST /api/v1/posts/{post_id}/comments`<br>`GET /api/v1/tags` | `create_post_service` |
| 事件竞猜与预测市场 | `GET /api/v1/users/me/balance`<br>`POST /api/v1/events/{event_id}/bets`<br>`GET /api/v1/users/me/bets`<br>`GET /api/v1/users/me/ledger`<br>`POST /api/v1/admin/events/{event_id}/settle`<br>`POST /api/v1/admin/events/{event_id}/refund`<br>`GET /api/v1/users/me/positions` | `place_bet_service`<br>`settle_event_service` |
| 两个项目共用的基础能力 | — | `get_session` |
...
## 本阶段新增功能：给数据库阶段补足两个查询视图和两个事务 service


> **实现边界：** 不增加 repository 层；先把 service 与事务边界真正掌握。
**建议完成顺序：** 先迁移 CRUD，再写查询功能，最后写多表事务。
...
| 项目 | 新增或修改的后端路由 | 新增或修改的后端函数 |
|---|---|---|
| 个人博客系统 | `GET /api/v1/posts/{post_id}/navigation` | `build_post_navigation` |
| 事件竞猜与预测市场 | `GET /api/v1/users/me/dashboard` | `build_user_dashboard` |
| 两个项目共用的基础能力 | — | — |
...
## 本阶段新增功能：Vue 迁移时增加两个轻量后端读取接口


**建议完成顺序：** 后端先稳定返回模型，Vue 再用 `ref` 与 `v-if` 展示。
...
| 项目 | 新增或修改的后端路由 | 新增或修改的后端函数 |
|---|---|---|
| 个人博客系统 | `GET /api/v1/posts/export.jsonl`<br>`GET /api/v1/posts/events`<br>`POST /api/v1/posts/{post_id}/publish（后台任务升级）` | `write_publish_audit` |
| 事件竞猜与预测市场 | `GET /api/v1/events/{event_id}/stream`<br>`GET /api/v1/events/export.jsonl` | `send_bet_confirmation` |
| 两个项目共用的基础能力 | — | — |
...
| 项目 | 新增或修改的后端路由 | 新增或修改的后端函数 |
|---|---|---|
| 个人博客系统 | — | `test_list_posts_returns_summaries_only`<br>`test_post_pagination_uses_filtered_total`<br>`test_get_missing_post_returns_404`<br>`test_create_post_uses_current_user_as_author`<br>`test_other_author_cannot_update_post`<br>`test_markdown_too_large_leaves_no_partial_file`<br>`test_comment_on_unpublished_post_is_rejected`<br>`seed_users_and_posts`<br>`test_create_post_then_fetch`<br>`test_upload_too_large_creates_nothing` |
| 事件竞猜与预测市场 | — | `test_place_bet_updates_all_four_records`<br>`test_same_idempotency_key_does_not_charge_twice`<br>`test_same_key_with_different_body_returns_409`<br>`test_insufficient_balance_rolls_back_everything`<br>`test_expired_event_rejects_bet_with_fixed_clock`<br>`test_settlement_is_idempotent`<br>`test_refund_creates_exactly_one_credit_per_bet`<br>`fixed_clock_override`<br>`test_idempotent_bet_replay`<br>`test_insufficient_balance_is_atomic`<br>`test_refund_is_idempotent` |
| 两个项目共用的基础能力 | — | `create_test_app`<br>`create_test_user`<br>`login_and_get_headers`<br>`test_empty_patch_changes_nothing`<br>`test_client fixture`<br>`build_auth_headers` |
...
## 本阶段新增功能：把测试阶段扩成真正的函数密集阶段


> **实现边界：** 优先验证失败后数据不变，不追求无意义覆盖率。
**建议完成顺序：** fixture→正常路径→冲突/权限→事务不变量。
...
### 后端函数卡：`test_settlement_is_idempotent`
...
| 项目 | 新增或修改的后端路由 | 新增或修改的后端函数 |
|---|---|---|
| 个人博客系统 | `GET /api/v1/posts/{post_id}/download`<br>`POST /api/v1/preferences/page-size`<br>`POST /api/v1/posts/{post_id}/reading-events（响应Header实验）` | — |
| 事件竞猜与预测市场 | `GET /api/v1/admin/events/{event_id}/report.csv` | — |
| 两个项目共用的基础能力 | — | `require_scopes` |
...
## 本阶段新增功能：把高级响应路由逐条落到真实前端


**建议完成顺序：** 后端函数先行→`/docs` 验证→最小前端入口→错误与边界测试。


#### 接口说明卡：`GET /api/v1/posts/{post_id}/download`
...
#### 接口说明卡：`POST /api/v1/preferences/page-size`
...
#### 接口说明卡：`GET /api/v1/admin/events/{event_id}/report.csv`
...
| 个人博客系统 | — | — |
| 事件竞猜与预测市场 | — | — |
| 两个项目共用的基础能力 | `GET /health/live`<br>`GET /health/ready`<br>`GET /internal/version` | `get_settings`<br>`lifespan`<br>`check_database_ready`<br>`load_minimal_seed_data`<br>`create_shared_http_client / close_shared_http_client` |
...
## 本阶段新增功能：补足 Settings 与 lifespan 的资源函数


> **实现边界：** 生产不自动创建默认管理员，不在 lifespan 执行大型迁移。
**建议完成顺序：** Settings→资源函数→lifespan→live/ready。
...
| 项目 | 新增或修改的后端路由 | 新增或修改的后端函数 |
|---|---|---|
| 个人博客系统 | `WS /ws/posts/{post_id}/discussion` | — |
| 事件竞猜与预测市场 | `WS /ws/markets` | — |
| 两个项目共用的基础能力 | — | `ConnectionManager.connect`<br>`ConnectionManager.disconnect`<br>`ConnectionManager.broadcast` |
...
## 本阶段新增功能：补齐 WebSocket 端点与连接管理函数


**建议完成顺序：** 后端函数先行→`/docs` 验证→最小前端入口→错误与边界测试。


#### 接口说明卡：`WS /ws/posts/{post_id}/discussion`
...
#### 后端函数说明卡：`ConnectionManager.connect`
- **函数类型**：连接管理函数
...
#### 后端函数说明卡：`ConnectionManager.broadcast`
- **函数类型**：连接管理函数
- **由谁调用**：评论保存成功后
- **接收什么**：post_id和公开消息。
- **返回什么**：无。
...
| 项目 | 新增或修改的后端路由 | 新增或修改的后端函数 |
|---|---|---|
| 个人博客系统 | `POST /api/v1/admin/webhook-subscriptions`<br>`GET /api/v1/admin/webhook-subscriptions`<br>`DELETE /api/v1/admin/webhook-subscriptions/{subscription_id}` | `enqueue_webhook_event` |
| 事件竞猜与预测市场 | `POST /api/v1/webhooks/oracle-results`<br>`GET /api/v1/admin/oracle-results/{oracle_result_id}`<br>`POST /api/v1/admin/oracle-results/{oracle_result_id}/approve` | — |
| 两个项目共用的基础能力 | — | — |
...
## 本阶段新增功能：补齐 Webhook 订阅和 Oracle 接收函数


**建议完成顺序：** 后端函数先行→`/docs` 验证→最小前端入口→错误与边界测试。


#### 接口说明卡：`POST /api/v1/admin/webhook-subscriptions`
...
#### 接口说明卡：`DELETE /api/v1/admin/webhook-subscriptions/{subscription_id}`
...
#### 接口说明卡：`POST /api/v1/webhooks/oracle-results`
...
#### 接口说明卡：`GET /api/v1/admin/oracle-results/{oracle_result_id}`
...
| 项目 | 新增或修改的后端路由 | 新增或修改的后端函数 |
|---|---|---|
| 个人博客系统 | — | `smoke_test_blog` |
| 事件竞猜与预测市场 | — | `smoke_test_prediction_market` |
| 两个项目共用的基础能力 | — | `validate_required_settings`<br>`run_database_migrations`<br>`seed_initial_admin`<br>`smoke_check_backend`<br>`verify_frontend_build`<br>`verify_backup_restore`<br>`wait_for_readiness` |
...
## 本阶段新增功能：部署阶段用脚本函数增加实践量，而不是硬造业务接口


> **实现边界：** 所有脚本先在测试/预发布环境练习。
**建议完成顺序：** 配置校验→备份→迁移→启动→ready→smoke→切流量。
